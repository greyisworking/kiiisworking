...
const validTypes = ['leader', 'ready', 'exploring', 'starting'];

export default async function handler(request) {
  const url = new URL(request.url);
  const typeParam = url.searchParams.get('type') || 'leader';
  const type = validTypes.includes(typeParam) ? typeParam : 'leader';
  ...
  const modifiedHtml = resultHtml
    ...
    .replace(/<meta property="og:image" content="[^"]*" id="og-image">/,
      `<meta property="og:image" content="${baseUrl}/og/${type}.png" id="og-image">`)
    .replace(/<meta property="og:url" content="[^"]*" id="og-url">/,
      `<meta property="og:url" content="${baseUrl}/result/${type}" id="og-url">`)
    ...
    .replace(/<meta name="twitter:image" content="[^"]*" id="tw-image">/,
      `<meta name="twitter:image" content="${baseUrl}/og/${type}.png" id="tw-image">`)
    .replace(/<title>[^<]*<\/title>/, 
